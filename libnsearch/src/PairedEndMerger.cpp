...
PairedEndMerger::PairedEndMerger( int minOverlap, double minIdentity )
  : mMinOverlap( minOverlap ), mMinIdentity( minIdentity )
{

}

bool PairedEndMerger::Merge( Sequence& merged, const Sequence &fwd, const Sequence &rev ) const {
  overlapInfo overlap;
...
int PairedEndMerger::ComputeOverlapScore( const char *sequence1, const char *sequence2, size_t len ) const {
  int score = 0;
 ...
 *
 */
bool PairedEndMerger::FindBestOverlap( overlapInfo &overlap, const std::string &sequence1, const std::string &sequence2 ) const {
...
bool PairedEndMerger::IsStaggered( const overlapInfo& overlap ) const {
  return overlap.pos2 > 0;
}

void PairedEndMerger::PrintOverlap( const Sequence &seq1, const Sequence &seq2, const overlapInfo &overlap ) const {
