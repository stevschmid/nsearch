...
TEST_CASE( "ExtendAlign" ) {
  ...
  SECTION( "Forwards extend" ) {
    ...
    score =
      ea.Extend( a, b, &bestA, &bestB, &cigar, AlignmentDirection::fwd, 3, 3 );
    REQUIRE( bestA == 3 );
    REQUIRE( bestB == 3 );
    REQUIRE( cigar.ToString() == "1M" );

    score =
      ea.Extend( a, b, &bestA, &bestB, &cigar, AlignmentDirection::fwd, 4, 4 );
    REQUIRE( bestA == 4 );
    REQUIRE( bestB == 4 );
  ...
  SECTION( "Backwards extend" ) {
    ...
    score =
      ea.Extend( a, b, &bestA, &bestB, &cigar, AlignmentDirection::rev, 1, 0 );
    REQUIRE( bestA == 1 );
    REQUIRE( bestB == 0 );
  }
}
